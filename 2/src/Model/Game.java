...
public class Game {
    ...
    public static void game() {
        if (win(p1.getXout(), p1.getYout(), p1.getSymb())) {
            alert("Победил" + p1.getType());
        }

        if (!fullMap()) {
            alert("Карта заполнена - ничья!");
        }
        ...
        if (win(p2.getXout(), p2.getYout(), p2.getSymb())) {
            alert("Победил" + p2.getType());
        }
    ...
    static void action(ActionEvent actionEvent) {
        for(int x = 0; x < map.getMapArr().length; ++x) {
            for(int y = 0; y < map.getMapArr().length; ++y) {
                if (actionEvent.getSource().equals(map.getMapArr()[x][y]) && map.getMapArr()[x][y].getText().equals(map.getEmpty())) {
                    p1.turn(x, y, map);
                }
            }
        }
    ...
    private static boolean win(int x, int y, String symb) {
        if (checkLine(x, y - map.getDOT_TO_WIN() + 1, 0, 1, symb)) {
            return true;
        } else if (checkLine(x - map.getDOT_TO_WIN() + 1, y - map.getDOT_TO_WIN() + 1, 1, 1, symb)) {
            return true;
        } else if (checkLine(x - map.getDOT_TO_WIN() + 1, y, 1, 0, symb)) {
            return true;
        } else {
            return checkLine(x - map.getDOT_TO_WIN() + 1, y + map.getDOT_TO_WIN() - 1, 1, -1, symb);
        }
    }
        ...
        for(int j = 0; j < map.getDOT_TO_WIN(); ++j) {
            ...
            for(int i = 0; i < map.getDOT_TO_WIN(); ++i) {
                if (cx + i * vx < map.getSIZE() && cy + i * vy < map.getDOT_TO_WIN() && cx + i * vx > -1 && cy + i * vy > -1) {
                    if (!map.getMapArr()[cx + i * vx][cy + i * vy].getText().equals(symb)) {
                        break;
                    }

                    ++w;
                    if (w == map.getDOT_TO_WIN()) {
                        return true;
                    }
                }
            }
    ...
    private static boolean fullMap() {
        for(int i = 0; i < map.getSIZE(); ++i) {
            for(int j = 0; j < map.getSIZE(); ++j) {
                if (map.getMapArr()[j][i].getText().equals(map.getEmpty())) {
                    return true;
                }
            }
        }
    ...
    static {
        ...
        p1 = new HumanPlayer("X", "Человек");
        p2 = new AiPlayer("O", "AI");
        p3 = new HumanPlayer("O", "Человек");
    }
}